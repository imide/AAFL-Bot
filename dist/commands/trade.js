...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName('team').setDescription('Team to trade with.').setRequired(true).addChoices({
...
async function execute(interaction) {
    if (interaction.inCachedGuild()) {
        ...
        const teamOwnerReq = guild.members.cache.find((member) => member.roles.cache.has(teamRole.id)) &&
        ...
        const ownerTradeDM = new discord_js_1.EmbedBuilder()
            .setAuthor({
            ...
            .setDescription(`**${teamOwnerInit.user.username}** of the **${teamName1}** has offered to trade **${player1.user.username}** to your team for **${player2.user.username}**. \n\n You can contact the owner of the **${teamName1}** to discuss the trade, or accept it right now. \n\n **Note:** If you accept this trade, you will be unable to reverse it. This trade request will expire in 48 hours.`)
            .setTimestamp()
            .setFooter({
            text: 'AAFL Transaction Manager'
        });
        const ownerTradeDMRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('acceptTrade').setLabel('Accept Trade').setStyle(discord_js_1.ButtonStyle.Success).setEmoji('✅'), new discord_js_1.ButtonBuilder().setCustomId('denyTrade').setLabel('Deny Trade').setStyle(discord_js_1.ButtonStyle.Danger).setEmoji('❌'));
        const transactionEmbed = new discord_js_1.EmbedBuilder()
            .setAuthor({
            name: 'AAFL Transaction Manager',
            iconURL: 'https://cdn.discordapp.com/attachments/1008505934483558421/1094751459972743198/aafl_logo.png'
        })
            .setTitle('Trade Confirmed')
            .setDescription(`**Trade Notice** 
    \n
