...
export const data = new SlashCommandBuilder()
  ...
  .addUserOption((option) =>
    option
      .setName("user")
      .setDescription("User of the person to trade. (your player)")
      .setRequired(true)
  )
  .addUserOption((option) =>
    option
...
export async function execute(interaction: ChatInputCommandInteraction) {
  if (interaction.inCachedGuild()) {
    ...
    const teamName1 = teamOwnerInit.roles.cache.some(
      (role) => role.name === "Pittsburgh Steelers"
    )
      ? "Pittsburgh Steelers"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "New Orleans Saints"
        )
      ? "New Orleans Saints"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "Tampa Bay Buccaneers"
        )
      ? "Tampa Bay Buccaneers"
      : teamOwnerInit.roles.cache.some((role) => role.name === "Buffalo Bills")
      ? "Buffalo Bills"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "Atlanta Falcons"
        )
      ? "Atlanta Falcons"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "New England Patriots"
        )
      ? "New England Patriots"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "Philadelphia Eagles"
        )
      ? "Philadelphia Eagles"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "Kansas City Chiefs"
        )
      ? "Kansas City Chiefs"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "Baltimore Ravens"
        )
      ? "Baltimore Ravens"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "San Francisco 49ers"
        )
      ? "San Francisco 49ers"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "Seattle Seahawks"
        )
      ? "Seattle Seahawks"
      : teamOwnerInit.roles.cache.some(
          (role) => role.name === "Tennessee Titans"
        )
      ? "Tennessee Titans"
      : "Administrative Team";
    ...
    const teamOwnerReq =
      ...
      guild.members.cache.find((member) =>
        member.roles.cache.has("Team Owner")
      )!;
    const channel =
      teamOwnerReq!.user.dmChannel || (await teamOwnerReq!.user.createDM());
      ...
      **${teamName1}** to recieve: **${player2}** 
      \n\n 
        **${teamName2}** to recieve: **${player1}**
        \n\n
      **Total Players:** (${teamName1}) ${
          interaction.guild?.roles.cache.find((role) => role.name === teamName1)
            ?.members.size
        }
      \n\n
      **Total Players:** (${teamName2}) ${
          interaction.guild?.roles.cache.find((role) => role.name === teamName2)
            ?.members.size
        }
      \n\n
    ...
    if (teamName1 === teamName2) {
      ...
      collector.on("collect", async (i) => {
        if (i.customId === "acceptTrade") {
          ...
          await transactionsChannel.send({ embeds: [transactionEmbed] });
